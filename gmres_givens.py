...
def apply_givens_rotation(h, cs, sn):
    """Apply Givens rotation to a 2D vector."""
    v1 = cs * h[0] + sn * h[1]
    v2 = -sn * h[0] + cs * h[1]
    return v1, v2
...
def gmres_givens(A, b, x0=None, tol=1e-8, max_iter=100):
    """
    GMRES implementation using progressive Givens rotations for the QR factorization.
    Builds the approximate solution at each step to check convergence.
    """
    n = len(b)
    if x0 is None:
        x = np.zeros(n)
    ...
    for j in range(max_iter):
        ...
        # Build the approximate solution to check convergence
